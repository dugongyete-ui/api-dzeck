...
async function addUserWithPassword(usernameInputId, passwordInputId) {
  ...
  try {
      ...
      const displayPassword = data.password || password || username;

      const newRowHtml = `
      <tr id="user_${token}" class="user">
          <td class="py-1 fond-bold inter darkblue text-lg border-b border-slate-800">
              <div>
                  <div class="flex justify-start mb-3">
                      <img id="delete_icon" src="/static/img/delete(Anggara).png" width="32" class="inline-block mx-2 p-0.5 hover:brightness-125" alt="Delete" onclick="deleteUserFromDB('${username}', 'user_${token}');"/>
                      <input type="text" id="username_${token}" name="username_${token}" class="input outline-none py-1 px-2 rounded-lg inter" placeholder="username_${token}" value="${username}" onchange="showElement('ELEMENT')" readonly>
                      <input type="hidden" name="password_${token}" value="${password}">
                  </div>
              </div>
          </td>
          <td class="py-1 fond-bold inter darkblue text-lg align-top">
              <button type="button" id="edit_user_${token}" class="text-white darkblue_bg outline-none focus:outline-none inter rounded-lg text-md px-4 py-1 text-center inline-flex items-center me-2 hover:opacity-95" onclick="enableEditing('username_${token}'); replaceElement(this.id, 'confirm_user_${token}'); focusOnInput('username_${token}')">
                  <img src="/static/img/edit(PixelPerfect).png" class="w-3.5 h-3.5 me-2"/>
                  <b>Edit</b>
              </button>
              <button type="button" id="confirm_user_${token}" class="text-white darkgreen_bg outline-none focus:outline-none inter rounded-lg text-md px-4 py-1 text-center inline-flex items-center me-2 hover:opacity-95 hidden" onclick="disableEditing('username_${token}'); replaceElement(this.id, 'edit_user_${token}')">
                  <img src="/static/img/check(iconmasadepan).png" class="w-3.5 h-3.5 me-2"/>
                  <b>Confirm</b>
              </button>
          </td>
      ...
      const addUserRow = document.getElementById('add_user');
      addUserRow.insertAdjacentHTML('afterend', newRowHtml);

      const infoDiv = document.getElementById('new_user_created_info');
      const createdUsernameDisplay = document.getElementById('created_username_display');
      const createdPasswordDisplay = document.getElementById('created_password_display');
      if (infoDiv && createdUsernameDisplay && createdPasswordDisplay) {
          createdUsernameDisplay.textContent = username;
          createdPasswordDisplay.textContent = displayPassword;
...
async function deleteUserFromDB(username, elementId) {
